...
import { DefaultTableData, TableData } from ".";
...
const defaultOrTable = (table: any, defaultTableData: any, key: string) => {
  try {
    let value: any = null;
    if (table.hasOwnProperty(key)) {
      value = table[key];
    } else if (defaultTableData.hasOwnProperty(key)) {
      value = defaultTableData[key];
    }
    return value;
  } catch (e) {
    return null;
